    ...
    >
      ...
      >
        <div className="px-6 py-5 border-b border-slate-100 flex items-start justify-between gap-4 shrink-0">
          <div>
            <h2 id="daily-transactions-title" className="text-lg font-bold text-slate-900">
            ...
            <tbody className="divide-y divide-slate-100">
              {transactions.map((t) => (
                <tr key={t.id} className="hover:bg-slate-50/60">
                  ...
                  <td className="px-4 py-2.5 whitespace-nowrap font-mono text-xs">{t.plat_nomor === "B0000XX" ? "Tanpa Plat" : t.plat_nomor || "-"}</td>
...
export function RekapDashboard({
  ...
  async function handleExportExcel() {
    ...
    try {
      ...
      const exportTotalKolom = {
        ...
        pendapatanKotor: exportResult.harian.reduce((a, h) => a + h.pendapatanKotor, 0),
        bagianKaryawan: exportResult.harian.reduce((a, h) => a + h.bagianKaryawan, 0),
        pendapatanBersih: exportResult.harian.reduce((a, h) => a + h.pendapatanBersih, 0),
      }
      const exportRataRata = {
        ...
        pendapatanKotor: exportTotalKolom.pendapatanKotor / exportResult.harian.length,
        bagianKaryawan: exportTotalKolom.bagianKaryawan / exportResult.harian.length,
        pendapatanBersih: exportTotalKolom.pendapatanBersih / exportResult.harian.length,
      }
  ...
  async function handleExportPdf() {
    ...
    try {
      ...
      doc.setFontSize(10)
      doc.text(`Periode: ${formatTanggalPanjang(exportDateFrom)} - ${formatTanggalPanjang(exportDateTo)}`, 14, 21)
      doc.text(
