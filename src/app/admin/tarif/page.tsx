...
export default async function TarifPage() {
  ...
  const { data: jenisKendaraan, error } = await supabase
    ...
    .select("id, kategori, ukuran, tarif_default, aktif")
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-2 text-base">
            Atur harga cuci per jenis kendaraan. Klik tarif untuk mengedit langsung.
          </p>
        </div>
        <div className="shrink-0 hidden sm:flex items-center gap-2 bg-indigo-50/80 border border-indigo-200/60 rounded-xl px-4 py-2.5">
          ...
          <span className="text-xs font-medium text-indigo-700">Klik angka tarif untuk edit</span>
        </div>
      </div>
