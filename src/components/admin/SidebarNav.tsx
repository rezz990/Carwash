...
export function MobileNav() {
  ...
  return <>
    <button type="button" onClick={() => setOpen(true)} className="icon-button" aria-label="Buka navigasi" aria-haspopup="dialog" aria-expanded={open}><EllipsisVertical size={22} /></button>
    {open && <Modal title="Navigasi" onClose={() => setOpen(false)}>
      <nav aria-label="Navigasi utama" className="space-y-1">
        {navItems.map(item => {
          const active = item.exact ? pathname === item.href : pathname === item.href || pathname.startsWith(item.href + "/")
          return <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} onClick={() => setOpen(false)}
            className={cn("flex min-h-12 items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold", active ? "bg-yellow-400 text-slate-900" : "text-slate-600 hover:bg-slate-50")}>
